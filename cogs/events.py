            ...
            if ctx.content.startswith('**🙏 |'):  # checks if its a pray message
                ...
                if ctx.content.split('**')[3] == '!Louis':
                    ...
                    if (datetime.datetime.now(datetime.timezone.utc) - last_pray[0]["timestamp"])\
                        ...
                        if level_up:
                            await self.distribute_rewards(user["id"] if user["id"] else user["username"], user_lvl + 1)  # if id exists use it cus more secure and stuff
                            await self.bot.hdb.set_user_xp(user["id"] if user["id"] else user["username"], 0)  # reset xp
