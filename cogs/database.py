...
class Database(commands.Cog):
    ...
    async def add_pray(self, username):
      await self.bot.db.execute("""INSERT INTO pray_logs (username, timestamp) VALUES ($1, $2)""", username, datetime.datetime.utcnow())

      await self.bot.db.execute("""UPDATE users SET pray_count = pray_count + 1 WHERE username = $1""", username)  # update count
    ...
    async def get_count_by_time(self, user, timetype):
      return await self.bot.db.fetch("""SELECT * FROM pray_logs WHERE timestamp > timestamp - ('1' || $1)::interval AND username = $2""", timetype,  user.name)
    
    async def get_daily_count(self, user):
      return await self.bot.db.fetch("""SELECT * FROM pray_logs WHERE timestamp > CURRENT_DATE::timestamp AND username = $1""", user.name)
