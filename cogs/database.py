...
class Database(commands.Cog):
    ...
    async def get_count_by_time(self, user, timetype):
        return await self.bot.db.fetch(
            """SELECT * FROM pray_logs WHERE timestamp > timestamp - ($1::interval):timestamp AND user_id = $2""",
            timetype, user.id)
    ...
    async def get_lb_users_by_time(self, timetype):
        return await self.bot.db.fetch(
            """SELECT * FROM pray_logs WHERE timestamp > timestamp - (('1' || $1)::interval)::timestamp""", timetype)
